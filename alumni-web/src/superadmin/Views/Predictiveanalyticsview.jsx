...
const Predictiveanalyticsview = ({
  ...
  const changeVal = values.length > 1 ? overviewTrend[overviewTrend.length - 1].value - overviewTrend[0].value : 0;
          ...
          <div className="pa-overview-container">
            <div className="pa-chart-card">
              ...
              <div className="pa-chart-shell">
                <div className="pa-chart-y-axis">{yLabels.map((v) => <span key={v}>{v}</span>)}</div>
                <div className="pa-chart-main">
                  ...
                  <svg className="pa-chart-svg" viewBox="0 0 100 100" preserveAspectRatio="none">
                    <polygon points={upperPoints} fill="rgba(219,234,254,0.5)" style={{ opacity: animProgress }} />
                    <polygon points={lowerPoints} fill="rgba(219,234,254,0.2)" style={{ opacity: animProgress }} />
                    {animatedTrend.length > 1 && (
                      <polyline fill="none" stroke="#3B82F6" strokeWidth="1.5" strokeLinejoin="round" points={linePoints} vectorEffect="non-scaling-stroke" />
                    )}
                    {overviewTrend.map((d, i) => (
                      <circle key={d.year} cx={toX(i)} cy={toY(d.value)} r="1.8" fill="#FFF" stroke="#3B82F6" strokeWidth="1.2" vectorEffect="non-scaling-stroke" style={{ opacity: Math.max(0, Math.min(1, (animProgress - (i / (overviewTrend.length - 1))) / 0.1)) }} />
                    ))}
                  </svg>
                  <div className="pa-chart-x-axis">{overviewTrend.map((d) => <span key={d.year}>{d.year}</span>)}</div>
                </div>
              </div>
              <div className="pa-chart-summary">
                <div className="pa-summary-block"><span className="pa-summary-label">Current (2025)</span><strong>{overviewTrend[0].value}%</strong></div>
                <LuArrowRight size={24} color="#51A2FF" />
                <div className="pa-summary-block"><span className="pa-summary-label">Predicted (2030)</span><strong>{overviewTrend[overviewTrend.length - 1].value}%</strong></div>
            ...
            <div className="pa-ai-card"><AIInsightsCard overviewTrend={overviewTrend} departmentCards={departmentCards} selectedDepartmentData={null} /></div>
          </div>
        )}

        {/* PAGE 2: Departments List */}
        {showDeptList && (
            ...
            {departmentCards.map((card) => (
              <div key={card.key} className="pa-department-card" onClick={() => onDepartmentClick(card.key)}>
                ...
                <div className="pa-department-metrics">
                  ...
                  <div className="pa-metric-row"><span className="pa-metric-label">Predicted 2030</span><span className={`pa-metric-value accent ${card.color}`}>{card.predicted}%</span></div>
                  <div className="pa-metric-change"><span className="pa-trend-badge"><LuArrowUpRight size={12} /> +{card.change}%</span></div>
                </div>
          ...
          <div className="pa-overview-container pa-detail-container">
            <div className="pa-panel">
              ...
              <div className="pa-bar-legend">
                <div className="pa-bar-legend-item"><span className="pa-bar-legend-swatch current" /><span className="pa-summary-label">Current</span></div>
                <div className="pa-bar-legend-item"><span className="pa-bar-legend-swatch predicted" /><span className="pa-summary-label">Predicted</span></div>
              </div>
                ...
                {selectedDepartmentData.programs.map((prog) => (
                  <div key={prog.code} className="pa-overview-card">
                    <div className="pa-overview-card-top">
                      <div className="pa-overview-card-title"><span className="pa-program-code">{prog.code}</span><span className="pa-trend-badge">+{prog.change}%</span></div>
                      <div className="pa-overview-stats"><span className="pa-overview-values">{prog.current}% → {prog.predicted}%</span></div>
                    </div>
            ...
            <div className="pa-ai-card"><AIInsightsCard overviewTrend={overviewTrend} departmentCards={departmentCards} selectedDepartmentData={selectedDepartmentData} /></div>
          </div>
        )}
