...
// ── Content item card ─────────────────────────────────────────────────────────
// SYNCED FROM ADMIN: Uses content-item-image-wrapper, content-item-type-badge,
// content-item-tag, content-item-icon-only, content-item-body, content-item-divider,
// hover state, 13px icons, 85-char truncation, jobs-only-location meta.
const ContentItemCard = ({ item, type, onEdit, onArchive }) => {
  ...
  // FIX: corrected typo "announce ments" → "announcements"
  const typeColor = {
  ...
  const truncatedDesc = description?.length > 85 ? description.slice(0, 85) + '…' : description;

  return (
    <div
      className={`content-item-card ${hovered ? 'hovered' : ''}`}
      ...
      {showImage ? (
        <div className="content-item-image-wrapper">
          <img
            src={item.image_url}
            alt={item.title}
            className={`content-item-image ${hovered ? 'hovered' : ''}`}
            onError={(e) => { e.target.style.display = 'none'; }}
          />
          <div className="content-item-type-badge" style={{ background: typeColor }}>
            {typeIcon()}
          </div>
          {tagText && (
        ...
        <div className="content-item-icon-only" style={{ background: typeColor }}>
          {typeIcon()}
        </div>
      )}
      <div className="content-item-body">
        <h4 className="content-item-title">{item.title}</h4>
        <p className="content-item-description">{truncatedDesc}</p>
        ...
        {type === 'discounts' && item.valid_until && (
          <div className="content-item-meta">
