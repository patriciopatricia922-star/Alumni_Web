...
const resolveImages = (formData, existingUrls = []) => {
  ...
  const activeLandingSections = landingSections.filter(
    (l) => l.is_active !== false,
  );
      ...
      items
        ...
        .forEach((i) =>
          archived.push({
            ...
            description:
              (i[descField]?.substring(0, 100) ?? "") +
              (i[descField]?.length > 100 ? "..." : ""),
            createdBy: "Admin",
          }),
        );
  ...
  const handleCreateReward = async (formData) => {
    ...
    try {
      ...
      await logAction({
        action: "Create",
        module: "Rewards",
        description: `Created reward: ${formData.title}`,
        recordId: data[0]?.id,
        status: "Success",
      });
      await fetchRewards();
  ...
  const handleCreateLandingSection = async (formData) => {
    ...
    try {
      ...
      showToastMessage(
        "Failed to create landing section: " + err.message,
        "error",
      );
    }
  };
...
const handleUpdateEvent = async (id, formData) => {
  ...
  const handleUpdateLandingSection = async (id, formData) => {
    ...
    try {
      ...
      showToastMessage(
        "Failed to update landing section: " + err.message,
        "error",
      );
    }
  };
  ...
  const handleUpdateReward = async (id, formData) => {
    ...
    try {
      ...
      await logAction({
        action: "Update",
        module: "Rewards",
        description: `Updated reward: ${formData.title}`,
        recordId: id,
        status: "Success",
      });
      await fetchRewards();
  ...
  const handleArchive = async (type, id) => {
    ...
    try {
      ...
      await logAction({
        ...
        description: `Archived ${type.replace("landingpage", "landing section").slice(0, -1)}: ${itemTitle}`,
        recordId: id,
        status: "Success",
      });
...
function Superadminengagement() {
  ...
  return (
    <ContentManagementView
      ...
      onCreateReward={cm.handleCreateReward}   
onUpdateReward={cm.handleUpdateReward}
