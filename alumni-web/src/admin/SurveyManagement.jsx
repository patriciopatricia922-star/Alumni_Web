  ...
  sections: [
    ...
    },
    {
      ...
      questions: [
        ...
        {
          id: 2, type: "multiple", label: "Please specify any certiport certification earned", required: false,
          options: [
            ...
            "Microsoft Certified Fundamentals (Azure, Microsoft 365, Power Platform, etc.)",
            "Microsoft Certified Educator (MCE)",
            "Adobe Certified Professional (ACP) - Photoshop, Illustrator, InDesign, Premiere Pro, etc.",
            ...
            "Project Management Institute (PMI) - Project Management Ready Certification",
...
export default function SurveyManagement() {
  ...
  const updateQuestion = (sIdx, qIdx, patch) => {
    setSurvey((prev) => ({
      ...
      sections: prev.sections.map((sec, si) =>
        si !== sIdx
          ? sec
          : { ...sec, questions: sec.questions.map((q, qi) => (qi !== qIdx ? q : { ...q, ...patch })) }
      ),
    }));
  ...
  const duplicateQuestion = (sIdx, qIdx) => {
    ...
    setSurvey((prev) => {
      ...
      const duplicatedQ = { ...sec.questions[qIdx], id: Date.now(), label: `${sec.questions[qIdx].label} (Copy)` };
      const updatedQuestions = [...sec.questions];
      updatedQuestions.splice(qIdx + 1, 0, duplicatedQ);
      return { ...prev, sections: prev.sections.map((s, si) => (si !== sIdx ? s : { ...s, questions: updatedQuestions })) };
    });
  ...
  const addSection = () => {
    ...
    setSurvey((prev) => ({
      ...prev,
      sections: [...prev.sections, { id: Date.now(), title: newSectionTitle, description: "New section", questions: [] }],
    }));
  ...
  const startEditing = (question, sIdx, qIdx) => setEditingQuestion({ ...question, sIdx, qIdx });
