...
const StarRating = ({ value, onChange }) => {
  ...
  return (
    <div className="sc-stars">
      {[1,2,3,4,5].map(star => (
        <div key={star} className="sc-star" onClick={() => onChange(star)} onMouseEnter={() => setHovered(star)} onMouseLeave={() => setHovered(0)}>
        ...
        <div className="sc-content">
          ...
          <div className="sc-body">
            <div className="sc-card" ref={cardRef}>
              {errors.size > 0 && <div className="sc-error-banner"><strong>Please answer all required questions before proceeding.</strong></div>}
              <div>
                  ...
                  <div className="sc-radio-group">
                    {COMPETENCIES_OPTIONS.map(opt => (
                      <label key={opt} className="sc-checkbox-label">
                        <input type="checkbox" value={opt} checked={form.useful_competencies.includes(opt)} onChange={() => toggleCompetency(opt)} />{opt}
                      </label>
                    ))}
                  </div>
                </div>
                <div className="sc-skill-ratings">
                  {SKILL_RATINGS_KEYS.map(skill => (
                    <div key={skill} className="sc-skill-row">
                      ...
                      <StarRating value={form.skill_ratings[skill] || 0} onChange={r => setSkillRating(skill, r)} />
                    </div>
                  ))}
                </div>
                <div className="sc-field">
