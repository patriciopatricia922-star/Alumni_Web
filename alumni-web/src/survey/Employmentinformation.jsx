...
const INDEX_TO_FIELD = [
  ...
  'location_of_employment', 'monthly_income', 'reason_for_job',
  'other_reason_for_job', 'reasons_unemployed', 'other_reason_unemployed',
];
...
const EmploymentInformation = () => {
  ...
  const applyConfig = (config) => {
    ...
    empSection.questions.forEach((q, idx) => {
      ...
      if (fieldKey === 'type_of_industry'       && q.options) setIndustryOptions(q.options);
      if (fieldKey === 'employment_status'      && q.options) setEmploymentStatuses(q.options);
      if (fieldKey === 'reason_for_job'         && q.options) setReasonsForJob(q.options);
      if (fieldKey === 'reasons_unemployed'     && q.options) setUnemployedReasons(q.options);
      if (fieldKey === 'monthly_income'         && q.options) setMonthlyIncome(q.options);
  ...
  const validate = () => {
    ...
    if (form.employment_status === 'Other' && !form.other_employment_status.trim())
      e.add('other_employment_status');
    if (DEFAULT_EMPLOYED_STATUSES.includes(form.employment_status)) {
      ...
      if (form.reason_for_job === 'Other' && !form.other_reason_for_job.trim())
        e.add('other_reason_for_job');
    }
    if (DEFAULT_UNEMPLOYED_STATUSES.includes(form.employment_status)) {
      if (!form.reasons_unemployed) e.add('reasons_unemployed');
      if (form.reasons_unemployed === 'Other' && !form.other_reason_unemployed.trim())
        e.add('other_reason_unemployed');
    }
    return e;
  };
