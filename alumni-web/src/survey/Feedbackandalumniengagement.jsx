...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .fa-btn-save { height: 48px; padding: 0 24px; background: transparent; border: 1.24px solid rgba(255,255,255,0.3); border-radius: 10px; cursor: pointer; font-family: 'Arimo', Arial, sans-serif; font-size: 15px; font-weight: 400; color: rgba(255,255,255,0.8); transition: border-color 0.15s, color 0.15s; }
  ...
  @media (max-width: 767px) { .fa-content { margin-left: 0; } .fa-topbar { padding: 20px 16px 0; } .fa-badge { padding: 6px 12px; font-size: 10px; } .fa-bell { display: none; } .fa-title { padding: 12px 16px 0; font-size: 20px; } .fa-progress { margin: 10px 16px 0; padding: 14px 16px; } .fa-progress-row { font-size: 13px; } .fa-progress-label { font-size: 13px; } .fa-body { padding: 16px 16px 80px; } .fa-card { padding: 20px 16px 20px; gap: 24px; } .fa-section-title { font-size: 17px; } .fa-btn-prev { width: 100px; height: 44px; font-size: 14px; } .fa-btn-save { height: 44px; padding: 0 14px; font-size: 14px; } .fa-btn-submit { width: 100px; height: 44px; font-size: 14px; } }
  @media (max-width: 390px) { .fa-title { font-size: 17px; } .fa-btn-prev, .fa-btn-submit { width: 90px; font-size: 13px; } .fa-btn-save { padding: 0 10px; font-size: 13px; } }
...
const FeedbackAndAlumniEngagement = () => {
  ...
  // ── Form state — both sections merged ────────────────────────────────────
  ...
  useEffect(() => {
    const load = async () => {
      // Load both saved sections and merge
  ...
  const handleSave = async () => {
    // Save both sections independently so existing keys in Supabase are preserved
    ...
    await Promise.all([
      saveSectionProgress('feedback_university', feedbackPayload),
      saveSectionProgress('alumni_engagement',   engagementPayload),
    ]);
    setSaveToast(true);
        ...
        <div className="fa-content">
          <div className="fa-header">
            ...
            <div className="fa-progress">
              ...
              <div className="fa-progress-track">
                <div className="fa-progress-fill" style={{ width: `${PROGRESS_PCT}%` }} />
              </div>
          ...
          <div className="fa-body">
            <div className="fa-card" ref={cardRef}>
              {errors.size > 0 && <div className="fa-error-banner"><strong>Please answer all required questions before proceeding.</strong></div>}
              <div>
              ...
              <div className="fa-questions">
                ...
                {/* ── Feedback for University questions ─────────────────── */}
                ...
                <div className="fa-field">
                  ...
                  <textarea className="fa-textarea" placeholder="Enter your answer" value={form.suggestions} onChange={e => set('suggestions', e.target.value)} />
                </div>

                <div className="fa-divider" />

                {/* ── Alumni Engagement questions ────────────────────────── */}
                <div className="fa-field">
