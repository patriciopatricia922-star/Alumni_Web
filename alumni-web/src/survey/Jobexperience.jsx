...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .je-btn-save { height: 48px; padding: 0 24px; background: transparent; border: 1.24px solid rgba(255,255,255,0.3); border-radius: 10px; cursor: pointer; font-family: 'Arimo', Arial, sans-serif; font-size: 15px; font-weight: 400; color: rgba(255,255,255,0.8); transition: border-color 0.15s, color 0.15s; }
  ...
  @media (max-width: 767px) { .je-content { margin-left: 0; } .je-topbar { padding: 20px 16px 0; } .je-badge { padding: 6px 12px; font-size: 10px; } .je-bell { display: none; } .je-title { padding: 12px 16px 0; font-size: 20px; } .je-progress { margin: 10px 16px 0; padding: 14px 16px; } .je-progress-row { font-size: 13px; } .je-progress-label { font-size: 13px; } .je-body { padding: 16px 16px 80px; } .je-card { padding: 20px 16px 20px; gap: 24px; } .je-section-title { font-size: 17px; } .je-btn-prev { width: 100px; height: 44px; font-size: 14px; } .je-btn-save { height: 44px; padding: 0 14px; font-size: 14px; } .je-btn-next { width: 100px; height: 44px; font-size: 14px; } }
  @media (max-width: 390px) { .je-title { font-size: 17px; } .je-btn-prev, .je-btn-next { width: 90px; font-size: 13px; } .je-btn-save { padding: 0 10px; font-size: 13px; } }
          ...
          <div className="je-header">
            ...
            <div className="je-progress">
              ...
              <div className="je-progress-track">
                <div className="je-progress-fill" style={{ width: `${PROGRESS_PCT}%` }} />
              </div>
                  ...
                  <div className="je-radio-group">
                    {TIME_TO_FIND_JOB_OPTIONS.map(opt => (
                      <label key={opt} className="je-radio-label">
                        <input type="radio" name="time_to_find_job" value={opt} checked={form.time_to_find_job === opt} onChange={() => set('time_to_find_job', opt)} />{opt}
                      </label>
                    ))}
                  </div>
                </div>
                ...
                <div className="je-field">
                  ...
                  <div className="je-radio-group">
                    {EMPLOYMENT_DURATION_OPTIONS.map(opt => (
                      <label key={opt} className="je-radio-label">
                        <input type="radio" name="employment_duration" value={opt} checked={form.employment_duration === opt} onChange={() => set('employment_duration', opt)} />{opt}
                      </label>
                    ))}
                  </div>
                  {form.employment_duration === 'Other' && (
                    <input className="je-other-input" type="text" placeholder="Please specify" value={form.other_employment_duration} onChange={e => set('other_employment_duration', e.target.value)} style={{ borderColor: errors.has('other_employment_duration') ? '#F87171' : undefined }} />
                  )}
                </div>
                ...
                <div className="je-field">
                  ...
                  <div className="je-radio-group">
                    {FIRST_JOB_OPTIONS.map(opt => (
                      <label key={opt} className="je-radio-label">
                        <input type="radio" name="first_job_source" value={opt} checked={form.first_job_source === opt} onChange={() => set('first_job_source', opt)} />{opt}
                      </label>
                    ))}
                  </div>
                  {form.first_job_source === 'Other' && (
                    <input className="je-other-input" type="text" placeholder="Please specify" value={form.other_first_job_source} onChange={e => set('other_first_job_source', e.target.value)} style={{ borderColor: errors.has('other_first_job_source') ? '#F87171' : undefined }} />
                  )}
                </div>
                  ...
                  <div className="je-radio-group">
                    {FACTORS_OPTIONS.map(opt => (
                      <label key={opt} className="je-checkbox-label">
                        <input type="checkbox" value={opt} checked={form.first_job_factors.includes(opt)} onChange={() => toggleFactor(opt)} />{opt}
                      </label>
                    ))}
                  </div>
                  {form.first_job_factors.includes('Other') && (
                    <input className="je-other-input" type="text" placeholder="Please specify" value={form.other_job_factors} onChange={e => set('other_job_factors', e.target.value)} style={{ borderColor: errors.has('other_job_factors') ? '#F87171' : undefined }} />
                  )}
                </div>
