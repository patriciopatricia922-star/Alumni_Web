...
import { saveSectionProgress, loadSectionData } from '../lib/shsSurveyProgress';
...
const PREV_ROUTE      = '/surveyshs/personal-background';
const NEXT_ROUTE      = '/surveyshs/section-3'; // update when Section 3 ships
...
const getRequiredFields = (form) => {
  ...
  if (form.status === 'Stopped') {
    ...
    if (form.stopped_reason === 'Other') required.add('stopped_reason_other');
  }
...
const EducationalBackgroundSHS = () => {
  ...
  // Section 2 of the SHS survey may have dynamic labels in the future;
  // the subscription is wired now so it's ready without any structural change.
  useEffect(() => {
    let cancelled = false;
    const init = async () => {
      setLoadingConfig(true);
      try {
        await loadSurveyConfig(true); // prime the cache; no SHS-specific labels yet
      } finally {
    ...
    const channel = subscribeToSurveyConfigChanges(async () => {
      await loadSurveyConfig(true); // refresh cache on admin change
    });
  ...
  // Clears all downstream fields so stale values from a previous selection
  // never leak into saved data or validation.
  ...
  const setPursuedNuBranch = useCallback((val) => {
    setForm((f) => ({
      ...
      // Reset level-2 branch
      pursued_other_school:  '',
      // Reset all shared detail fields
      nu_branch:             '',
      reason_nu:             '',
      reason_not_nu:         '',
      school_name:           '',
