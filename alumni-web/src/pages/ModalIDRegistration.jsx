...
/**
 * Modal-aware wrapper around the ID Registration flow.
 * Identical logic to AlumniIDRegistration.jsx — no Supabase calls here,
 * OCR happens in verifyAlumniID() just as before.
 * Instead of navigate('/signup'), calls onVerified(extractedData).
 */
const ModalIDRegistration = ({ onVerified, onSwitchToLogin, onClose }) => {
  ...
  useEffect(() => {
    if (!imageFile) return;
    (async () => {
      setStatus('scanning'); setErrorMsg(''); setExtractedData(null);
      try {
        const result = await verifyAlumniID(imageFile);
        if (result.verified) { setStatus('verified'); setExtractedData(result.extracted); }
        else                 { setStatus('failed');   setErrorMsg(result.reason); }
      } catch (err) {
        setStatus('failed');
  ...
  const startDetectionLoop = useCallback(() => {
    ...
    const analyse = () => {
      ...
      if (elapsed < GRACE_PERIOD_MS) {
        ...
        if (secondsLeft > 0) setCamGuide(`Get your Alumni ID ready... (${secondsLeft})`);
      ...
      const fX = Math.floor(vW * 0.06); const fY = Math.floor(vH * 0.20);
      const fW = Math.floor(vW * 0.88); const fH = Math.floor(vH * 0.60);
      const W = 160; const H = Math.round(160 * (fH / fW));
      ...
      const skinRatio = (() => {
        ...
        const xStart = Math.floor(W*0.20); const xEnd = Math.floor(W*0.80);
        const yStart = Math.floor(H*0.20); const yEnd = Math.floor(H*0.80);
        for (let y = yStart; y < yEnd; y++) {
          for (let x = xStart; x < xEnd; x++) {
            const i = (y*W+x)*4;
            const r = data[i], g = data[i+1], b = data[i+2];
            if (r>95 && g>40 && b>20 && r>g && g>b && (r-g)>15 && (r-b)>15) skinCount++;
            total++;
          }
        }
      ...
      if (!cardFillsFrame && density < 0.05) {
        ...
        setCamGuide('That looks like a hand — please show your Alumni ID'); stable = 0;
        ...
        setCamGuide('Hold still — keep the ID steady'); stable = Math.max(0, stable - 8);
      } else {
        stable++;
    ...
    try {
      ...
      setTimeout(() => {
        if (videoRef.current) {
          ...
          videoRef.current.onloadedmetadata = () => { videoRef.current.play(); startDetectionLoop(); };
        }
      }, 100);
    } catch (err) {
  ...
  const handleReset = () => {
    ...
    setStatus('idle'); setErrorMsg(''); setExtractedData(null);
    if (fileInputRef.current) fileInputRef.current.value = '';
  };

  // ── KEY DIFFERENCE: instead of navigate, call onVerified ──
  const handleNext = () => {
